					   ...
					   int *grcount,int *frcount,int *vmgcout) 
{
	...
	int kk=-1,ll=-1,ff=-1,gg=-1,vv=-1;//用kk记录物理机的索引值
	...
	if (xmlStrcmp(name, (const xmlChar *) "PM")==0) {
		if(nt==XML_READER_TYPE_ELEMENT) 
		{
			...
			p_pm->at_p_pmgrps;
			p_pm->countofpmgrp;*/
			/*将(gr1,gr2,gr3,gr5)分割成1 2 3 4数组*/
		...
		if(nt==XML_READER_TYPE_ELEMENT) 
		{
			...
			if(att!=NULL) {
				...
				if(p_pm!=NULL){
					...
					_xpthis->sd->vmlist[ll].at_pmid = INVI;//添加at_pmid的初始化，by sun in 8-15
...
void process_XML21_Node(xmlTextReaderPtr reader, int *swcount,int *pscount,int *sscount) 
{
	...
	/*****************************************************/
	/*3.读取交换机与交换机的连接信息：如果元素名称是SW，则获取属性ID，PORTNUM,LEV等值*/
	if (xmlStrcmp(name, (xmlChar *) "S2S")==0) {
		
		if(nt==XML_READER_TYPE_ELEMENT) ssc++;

		/*读取ID*/
		p_ss->id=getint_attrib(reader,"ID",ssc<0);/*原：p_ps 现：p_ss modified by lyq 2013.8.18 */
		/*读取SRC为PMID*/
		getstr_attrib(reader,"SRC",p_ss->sw_src_sid,ssc<0);
		/*读取目标交换机SID*/
		getstr_attrib(reader,"DST",p_ss->sw_dst_sid,ssc<0);
		/*读取带宽*/
		p_ss->bandwidth=(float)getdbl_attrib(reader,"BW",ssc<0);/*原：p_ps 现：p_ss modified by lyq 2013.8.18 */
		/*读取时延*/
		p_ss->delay=(float)getdbl_attrib(reader,"DELAY",ssc<0);/*原：p_ps 现：p_ss modified by lyq 2013.8.18 */
	}
...
#define isNum(x) (x>='0'&&x<='9')
	...
	if (xmlStrcmp(name, (xmlChar *) "AFFINITY")==0) {
		...
		{
			//读取固定值at到affinity_at by CC 10.17
